 ...
 *
 * Ten sam kanał („hej_maja/sygnal") obsługuje po drugiej stronie iOS
 * (`ios/Runner/AppDelegate.swift`, `AudioServicesPlaySystemSound`), więc
 * `flutter_beep` wypadł z projektu w całości. Bliźniaczy plik stoi w forku
 * `pliki/heymaya_.../android/.../MainActivity.kt` - zmiany trzymamy zgodne.
 *
...
class MainActivity : FlutterActivity() {
  ...
  //
  //UWAGA: wersja androidowa aplikacji mieszka w forku
  //`pliki/heymaya_.../` i to TAM dobierany jest ton potwierdzenia
  //(15.08.2026: TONE_PROP_PROMPT). Tutaj, w `hi_bees`, liczy się wyłącznie
  //dźwięk iOS - system 1116 z `ios/Runner/AppDelegate.swift` - więc tej stałej
  //NIE zrównujemy z forkiem; zostaje jako zaplecze na wypadek, gdyby `hi_bees`
  //wrócił na Androida. Inne sensowne wartości, gdyby ten nie pasował:
  //  TONE_PROP_ACK        - dwutonowe "przyjąłem"
  //  TONE_PROP_BEEP2      - podwójny beep
  //  TONE_CDMA_ONE_MIN_BEEP - to, co grało na Androidzie przez flutter_beep
  private val tonPotwierdzenia = ToneGenerator.TONE_PROP_BEEP
  ...
  private fun zagrajSygnal(): String {
    ...
    for (strumien in strumienieDoProby) {
      val powod = sprobujTon(strumien) ?: return "ok: ton na ${nazwaStrumienia(strumien)}"
      powody += powod
    }
