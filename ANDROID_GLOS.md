...
- Android: `ToneGenerator`, 150 ms, głośność 80/100. Zmiana tonu = jedna stała
  `tonPotwierdzenia` w `MainActivity.kt` (alternatywy wypisane w komentarzu
  obok). Docelowy ton wybrany 15.08.2026 → `TONE_PROP_PROMPT` w
  `pliki/heymaya_…`, patrz 5.5.
...
## 5.4. Wynik testu (15.08.2026) i **świadoma różnica między drzewami**

Test na urządzeniach: iPhone 15 (`hi_bees`) → `ok: dźwięk systemowy iOS 1116`;
Samsung S7 (`heymaya`) → `ok: ton na media`. Czyli **kanał działa po obu
stronach, plik zapasowy nie poszedł w ruch** — zgłoszenie „i tak gra
`listening.wav`" brało się z brzmienia: `TONE_PROP_BEEP` to sinus ~0,15 s,
czyli bliźniak `listening.wav` (sinus 433 Hz, 0,15 s). Nie dało się ich odróżnić
ze słuchu.

**Nieporozumienie do zapamiętania:** `AndroidSoundIDs` z `flutter_beep` to były
po prostu stałe `ToneGenerator` przekazywane dalej — usunięcie pakietu NIE
zabrało żadnego dźwięku, wszystkie są w zasięgu jednej stałej.

## 5.5. Wybrany ton (15.08.2026) — i dlaczego drzewa zostają różne

Po przymiarkach na telefonie wybrany ton to **`TONE_PROP_PROMPT`** i stoi on
w **`pliki/heymaya_…` — to jest wersja androidowa aplikacji**. `hi_bees` jest
wydawany na iOS, więc liczy się w nim wyłącznie dźwięk systemowy **1116**
z `ios/Runner/AppDelegate.swift`; androidowa stała w
`android/…/MainActivity.kt` zostaje na `TONE_PROP_BEEP` jako zaplecze i **nie
jest błędem do „naprawienia" synchronizacją drzew** — kanał `hej_maja/sygnal`
trzymamy zgodny, dobór tonu nie.

Opis zwracany z kanału zawiera w forku NAZWĘ TONU
(`ok: ton TONE_PROP_PROMPT na media`), więc z telefonu widać, czy build jest
świeży i który wariant naprawdę gra.

## 5.6. Diagnostyka schowana przed publikacją (15.08.2026)

Testy androidowe zamknięte, więc wszystko, co dołożyliśmy na czas diagnozy,
wraca pod komentarz — **w OBU drzewach**. Mechanizmy zostają w kodzie, powrót
to zawsze odkomentowanie, nigdy pisanie od nowa:

| Gdzie | Co schowane |
|---|---|
| `lib/screens/voice_settings_screen.dart` | przełącznik **„Diagnostyka głosu"** (`globals.voiceDiagnostyka` i tak startuje `false`, więc surowy tekst z Vosk nie ma jak trafić na ekran) |
| `lib/screens/voice_settings_screen.dart` | pozycja **„Sygnał potwierdzenia komendy"** razem z polem `_opisSygnalu`; `SoundHelper.beep()` i `ostatniSygnal` NIETKNIĘTE, więc sam sygnał po komendzie gra dalej |
| `lib/helpers/vosk_engine.dart` | `_sladDiagnostyczny = false` — ślad `VOSK/ślad:` szedł przez `debugPrint`, ale liczenie porcji działo się co 0,2 s nasłuchu |
| `lib/screens/apiarys_screen.dart` (tylko `heymaya`) | ikona ekranu **POC Vosk**; w `hi_bees` schowana już 11.08.2026 |

**Pułapka przy ikonie POC:** import `vosk_poc_screen.dart` trzeba komentować
RAZEM z `IconButton`, inaczej `flutter analyze` zgłasza `unused_import`. Ekran
i trasa w `main.dart` zostają — wracają odkomentowaniem obu linii naraz.
